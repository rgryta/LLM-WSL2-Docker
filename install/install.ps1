...
$ignr = wsl -d $distro -u root -e sh -c "apt-get install -y apt-utils sudo curl systemd jq libssl-dev build-essential checkinstall zlib1g-dev git wget"

# Creating new user
wsl -d $distro -u root -e sh -c 'useradd -m -G sudo -p $(echo `openssl passwd -1 llm`) llm'
